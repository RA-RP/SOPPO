...
if model_path == "Qwen/Qwen3-1.7B" and method in {"dpo", "sspo", "staticpe"}:
    ...
    config.update({
        ...
        "learning_rate": 1e-5,
        "num_train_epochs": 1,
        "per_device_train_batch_size": round4_train_batch_sizes[method],
    ...
    if method == "sspo":
        config.update({
            ...
            "sspo_gamma_0": 1.0,
            "sspo_gamma_min": round(6113 / (6113 + 20785), 4),
            "sspo_prior": 0.5,
    ...
    if peft not in ["full", "lora", "q-lora"]:
        raise ValueError("Round4 supports --peft full, lora, or q-lora.")
    ...
    yaml_dir = f"./examples/train/{get_backbone_name(model_path)}/{method}/fb0.1_ch0.1/"
    os.makedirs(yaml_dir, exist_ok=True)
    filepath = os.path.join(yaml_dir, f"{run_name}.yaml")
    with open(filepath, "w", encoding="utf-8") as file:
        yaml.safe_dump(config, file, default_flow_style=False, sort_keys=False)
    print(f"We have 1 Round4 {method} configuration.")
