#!/usr/bin/env bash
# Start the reviewed formal chain in one isolated background process group.
set -euo pipefail
...
[[ ! -e "$PID_FILE" && ! -e "$PGID_FILE" && ! -e "$STARTTIME_FILE" && ! -e "$ROUND3_RUN_ROOT/controller.json" ]] || {
    echo "ERROR: Round3 controller evidence already exists; refuse duplicate launch" >&2
    exit 1
}
mkdir -p "$CONTROL_DIR" "$ROUND3_LOG_ROOT"
setsid nohup bash "$SCRIPT_DIR/run_all.sh" \
    > "$ROUND3_LOG_ROOT/controller.log" 2>&1 < /dev/null &
pid=$!
pgid="$(ps -o pgid= -p "$pid" | tr -d '[:space:]')"
[[ "$pgid" == "$pid" ]] || { echo "ERROR: Round3 controller lacks isolated process group" >&2; exit 1; }
starttime="$(awk '{print $22}' "/proc/$pid/stat")"
printf '%s\n' "$pid" > "$PID_FILE"
printf '%s\n' "$pgid" > "$PGID_FILE"
printf '%s\n' "$starttime" > "$STARTTIME_FILE"
echo "Round3 controller started. Use status_all.sh for read-only evidence."
