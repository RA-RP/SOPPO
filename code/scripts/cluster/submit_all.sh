...
SMOKE_PARTITION="${SOPPO_SMOKE_PARTITION:-gpu_test}"
CPU_PARTITION="${SOPPO_CPU_PARTITION:-cpu}"
...
command -v scontrol >/dev/null || { echo "ERROR: scontrol is unavailable" >&2; exit 1; }
...
declare -a HELD_JOBS=()
submission_failed() {
    echo "ERROR: DAG submission stopped. The following jobs remain held and did not run:" >&2
    printf '  %s\n' "${HELD_JOBS[@]:-none}" >&2
    echo "Inspect them with squeue and cancel explicitly if desired." >&2
}
...
PRE="$LAST_JOB_ID"
submit headroom_gate -J soppo-headroom -p "$CPU_PARTITION" -N 1 -c 4 -t 01:00:00 \
...
LAMBDA="$LAST_JOB_ID"
submit static_select -J soppo-static-select -p "$CPU_PARTITION" -N 1 -c 4 -t 01:00:00 \
...
MAIN="$LAST_JOB_ID"
submit c_epsilon_prepare -J soppo-ceprep -p "$CPU_PARTITION" -N 1 -c 4 -t 01:00:00 \
...
CE_RAW="$LAST_JOB_ID"
submit c_epsilon_derive -J soppo-cederive -p "$CPU_PARTITION" -N 1 -c 8 -t 02:00:00 \
...
EVAL="$LAST_JOB_ID"
submit aggregate -J soppo-aggregate -p "$CPU_PARTITION" -N 1 -c 8 -t 02:00:00 \
...
payload = {
    ...
    "git_commit": commit,
    "submission_status": "held_complete",
    "jobs": jobs,
}
temporary = path + ".tmp"
...
value["submission_status"] = "released"
temporary = path + ".tmp"
with open(temporary, "w", encoding="utf-8") as handle:
...
os.replace(temporary, path)
PY
echo "Complete Slurm DAG submitted and released."
