#!/usr/bin/env python3
"""N-2 item 2: OPD H_bos + H_ood at step_080 (generation already backfilled).

Derived from cycle09_offkd_geometry.py; probes swapped to the per-checkpoint H
corpora, arm=opd, single step 80 (its MMLU-Pro trough, missing from the H grid).
dW track follows OPD convention: top32_approx(merged-base). Base references for H
probes do not exist (H is per-checkpoint), so they are built on the fly and cached.
...
RUN_ROOT = Path("/root/autodl-tmp/cycle09_n2/h80_opd")
MINI = c4.MINI_ROOT
MAIN_GRID = (0, 80)
...
def probe_tasks(
    step: int = 80,
    arm: str = "opd",
    domains: tuple[str, ...] = H_DOMAINS,
) -> list[dict[str, Any]]:
    tasks: list[dict[str, Any]] = []
    for dom in domains:
        for seed in H_SEEDS:
            tid = f"H_{arm}_{dom}__{c4.step_label(step)}__g{seed}"
            tasks.append({"task_id": tid, "probe_type": "H", "domain": dom,
                          "corpus": c4.generated_corpus_path("H", dom, seed, arm, step, c4.RUN_ROOT),
                          "generation_seed": seed, "reference": tid})
    return tasks
def _unused_probe_tasks() -> list[dict[str, Any]]:
    tasks: list[dict[str, Any]] = []
...
def checkpoint_model_path(arm: str, step: int) -> Path:
    if step == 0:
        return c4.BASE_MODEL
    if arm == "opd":
        root = Path("/root/autodl-tmp/cycle08_opd_trajectory/_merged_models")
    elif arm == "offkd":
        root = OFFKD_MERGED
    else:
        raise ValueError(f"unsupported H geometry arm: {arm}")
...
def preflight(
    ...
    for step in steps:
        if step == 0:
            adapters[step] = None
            continue
        checkpoint_model_path(arm, step)
        adapters[step] = offkd_adapter_dir(step) if arm == "offkd" else None
        if arm == "offkd" and adapters[step] is None:
...
def load_reference_profile(
    ...
    if not path.exists():
        raise FileNotFoundError(
            f"missing base reference {path}; run the H base-reference builder first")
...
def protocol_payload(
    ...
    return {
        ...
        "smoke": args.smoke,
        "dW": (
            "offkd_adapter_BA_fp32_only"
            if args.arm == "offkd"
            else "opd_top32_approx_merged_minus_base_fp32"
        ),
...
def run(args: argparse.Namespace) -> None:
    ...
    device = args.device
    layers = list(c4.LAYERS)
    tasks = probe_tasks(args.probe_step, args.arm, args.domains)
    if args.smoke:
        tasks = tasks[:1]
        args.steps = (0,) if args.probe_step == 0 else (0, args.probe_step)
        args.measurement_n = 4
        args.mini_root = MINI / "smoke_h_geometry"
        args.work_root = args.work_root / "smoke"
    steps = list(args.steps)
    adapters = preflight(args.arm, steps, tasks)
    ...
    for step in pending:
        current_model = (
            base_model
            if step == 0
            else camp.load_model(checkpoint_model_path(args.arm, step), device)
        )
        adapter_state = None
        adapter_scale = 0.0
        if step != 0 and args.arm == "offkd":
            adapter_state = load_file(adapters[step] / "adapter_model.safetensors")
            adapter_scale = camp.adapter_scaling(adapters[step])
        source_kind = (
            "base_identity"
            if step == 0
            else "offkd_clean_fp32_ba"
            if args.arm == "offkd"
            else "opd_top32_approx"
        )
                ...
                for layer in layers:
                    for module in c4.MODULES:
                        ...
                        if step == 0:
                            ...
                            merged_w = camp.module_at(current_model, layer, module).weight.detach().float()
                            base_w = camp.module_at(base_model, layer, module).weight.detach().float()
                            update = camp.top32_approx(
                                merged_w - base_w,
                                c4.stable_seed(42, ARM, step, layer, module),
                            )
...
def append_arm_rows(path: Path, new_rows: list[dict[str, Any]], fields: tuple[str, ...]) -> int:
    ...
    if path.exists():
        with open(path, newline="", encoding="utf-8") as handle:
            ...
            probe_key = "task_id" if "task_id" in (reader.fieldnames or ()) else "probe"
            new_keys = {(r.get("arm"), str(r.get("step")), r.get(probe_key)) for r in new_rows}
            old_rows = [r for r in reader
                        if (r.get("arm"), str(r.get("step")), r.get(probe_key)) not in new_keys]
    for row in new_rows:
...
def write_outputs(
    ...
    all_spectra_rows: list[dict[str, Any]] = []
    all_m2_rows: list[dict[str, Any]] = []
    all_theta_rows: list[dict[str, Any]] = []
    for step in args.steps:
        payload = completed[int(step)]
        all_spectra_rows.extend(payload["spectra"])
        all_m2_rows.extend(payload["m2"])
        all_theta_rows.extend(payload["theta"])
    ...
    cache_actual = {
        "spectra": len(all_spectra_rows),
        "m2": len(all_m2_rows),
        "theta": len(all_theta_rows),
    }
    if cache_actual != cache_expected:
        raise ValueError(
            f"cache row-count mismatch: expected={cache_expected}, actual={cache_actual}"
        )

    # The step-0 pass is an internal frozen-base reference for the H@80 corpus,
    # not an additional trajectory point.  Existing H rows contain only the
    # checkpoint that generated the probe text, with base quantities in-row.
    output_steps = {int(step) for step in args.steps if int(step) != 0} or {0}
    spectra_rows = [r for r in all_spectra_rows if int(r["step"]) in output_steps]
    m2_rows = [r for r in all_m2_rows if int(r["step"]) in output_steps]
    theta_rows = [r for r in all_theta_rows if int(r["step"]) in output_steps]
    expected = {
        "spectra": sum(expected_step_counts(len(tasks), step)["spectra"] for step in output_steps),
        "m2": sum(expected_step_counts(len(tasks), step)["m2"] for step in output_steps),
        "theta": sum(expected_step_counts(len(tasks), step)["theta"] for step in output_steps),
    }
    actual = {
    ...
    if actual != expected:
        raise ValueError(f"output row-count mismatch: expected={expected}, actual={actual}")

    spectra_name = (
        "R4_v2_spectra_backfill_opd.csv"
        if ARM == "opd"
        else "R4_v2_spectra_h_offkd.csv"
    )
    n_spectra = append_arm_rows(mini / spectra_name, spectra_rows, SPECTRA_FIELDS)

    from cycle09_r4_postprocess import drift_core, family

    base_sigma: dict[tuple[str, str, int, str], list[float]] = {}
    for row in all_spectra_rows:
        if int(row["step"]) == 0:
            key = (
                family(row["task_id"]),
                row["track"],
    ...
    manifest = {
        ...
        "dW_track": (
            "off-KD adapter B@A fp32 only; no merged-minus-base"
            if ARM == "offkd"
            else "OPD top-32 approximation of merged(fp32)-base(fp32)"
        ),
        ...
        "models": {
            str(step): str(checkpoint_model_path(ARM, int(step)))
            for step in args.steps
        },
        "adapters": {
            str(step): (
                str(adapters[int(step)])
                if adapters[int(step)] is not None
                else "not_applicable"
            )
            for step in args.steps
        },
    ...
    manifest_name = (
        "h80_opd_geometry_manifest.json"
        if ARM == "opd"
        else f"offkd_h_geometry_{c4.step_label(args.probe_step)}_manifest.json"
    )
    write_json_atomic(mini / manifest_name, manifest)
    print(
...
def main() -> None:
    ...
    parser.add_argument("--arm", choices=("opd", "offkd"), default="opd")
    parser.add_argument("--probe-step", type=int, default=80)
    parser.add_argument("--domains", default=",".join(H_DOMAINS))
    ...
    args.domains = tuple(value for value in args.domains.split(",") if value)
    if args.steps:
