#!/usr/bin/env python3
"""N-2 step 1b: X_offkd_math probe on the off-KD arm, ten points.

Derived from cycle09_offkd_geometry.py (codex) with the probe set swapped to the single
new fixed probe X_offkd_math; all numerics/protocol identical.
...
RUN_ROOT = OFFKD_ROOT / "geometry_xoffkd"
MINI = c4.MINI_ROOT
MAIN_GRID = (0, 5, 10, 20, 40, 80, 160, 320, 480, 624)
EXTENDED_GRID = (0, 5, 10, 20, 40, 80, 160, 320, 480, 624)
EPSILONS = (0.05, 0.01)
TAIL_RANKS = (32, 64, 128, 256)

FIXED_PROBES = {
    "X_offkd_math": ("X", "math", "corpora/fixed/X_offkd_math.jsonl", None),
}
SBOS_SEEDS = ()
...
def run(args: argparse.Namespace) -> None:
    ...
    if args.smoke:
        ...
        args.mini_root = MINI / "smoke_offkd"
    ...
    if path.exists():
        with open(path, newline="", encoding="utf-8") as handle:
            ...
            old_rows = [
                ...
                if not (row.get("arm") == ARM and row.get(probe_key) == "X_offkd_math")
            ]
    for row in new_rows:
...
def write_outputs(
    ...
    c4.write_csv_atomic(
        mini / "R4_v2_spectra_offkd.csv", spectra_rows, list(SPECTRA_FIELDS)
    )
    ...
    manifest = {
        ...
        "rows_written": {
            "spectra_offkd": len(spectra_rows),
            "m1": n_m1,
            "m2": n_m2,
            "theta": n_theta,
        },
    ...
    write_json_atomic(mini / "xoffkd_geometry_manifest.json", manifest)
    print(
