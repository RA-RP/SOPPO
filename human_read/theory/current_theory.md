...
- 最近更新：2026-08-19（整合 QA 决策，更新为 v0.2）
- 上一版本：v0.1（初始理论草稿）
...
在偏好标签有限时，与其把模型对每个无标签回复对的当前预测转化为硬或软伪目标，不如只用这些预测在群体层面估计两类偏好方向的 label encoding，并用少量真实偏好对锚定方向；如果这种更弱的结构监督有效，再把无标签回复对扩展为由 SFT 参考回复与当前策略在线回复构成的动态候选对。
...
两份初始材料描述的是同一研究方向的两个层次：

1. **第一阶段：静态无标签偏好对 MVP。** 在已有完整标签的 preference dataset 上人为隐藏大部分训练标签，只检验 label-encoding 结构监督本身是否提供有效信息。本阶段不引入 on-policy、SFT rollout、scheduler 或 token-level 扩展。
2. **第二阶段：动态 SFT 候选对扩展。** 只有第一阶段的核心命题成立后，才把无标签对替换为 SFT 参考回复与当前策略在线回复构成的方向未知候选对，恢复 DOCX 中“真实偏好锚定—监督数据在线成对化—偏好编码估计—动态联合优化”的完整闭环。

这种顺序保留了原始动态方法，但避免在首次验证时同时改变数据来源、采样策略和损失函数，从而使失败或成功更容易归因。
...
从带有真实方向的 preference dataset 构造
...
其中 $\lambda$ 控制无标签结构信号的强度。$L_1$ 距离是初始方案，最终是否优于 $L_2$、KL 或其他结构距离需要由消融验证。
...
3. **DPO + PE**：在 $D_L$ 上使用 DPO，同时对 $D_U$ 使用 $L_{\mathrm{PE}}$，不给单个无标签对生成训练目标。
4. **DPO-100%**：恢复 $D_U$ 的真实标签，作为 oracle upper bound，不作为同等监督预算下的竞争方法。

第一阶段可使用一个完整标注的 preference dataset，例如附件提出的 UltraFeedback。具体数据版本、过滤、划分和模型选择属于实验设计，必须在运行前固定。
...
的动态闭环。第二阶段检验的是结构监督在分布持续变化、候选对方向未知时能否继续成立，而不是第一阶段的必要组成部分。

## 8. 关键假设与可证伪预测

| 编号 | 假设 | 可观察预测 | 反驳或削弱条件 |
| --- | --- | --- | --- |
| H1 | 少量真实偏好对足以锚定共享的偏好方向 | DPO + PE 的测试集 Acc 高于 DPO-10%，且 Brier 不恶化 | 多切分、多随机种子下无稳定提升 |
| H2 | 群体级结构约束比逐样本伪目标更能抑制早期错误累积 | DPO + PE 优于公平实现的硬/软 Pseudo-target | Pseudo-target 持续持平或更优 |
| H3 | 结构收益来自无标签对的整体组织，而不只是置信度变尖 | Acc 与 Brier 同时改善，类别质量和预测分布未坍缩 | 仅置信度/熵下降，准确率或校准恶化 |
| H4 | 静态机制成立后可迁移到动态 SFT 候选对 | 第二阶段在受控增量实验中仍优于对应无结构基线 | 静态有效但在线候选分布下失效 |
...
- **重复次数**：3 个随机种子
- **显著性**：统计显著性 p < 0.05
- **最低有效性**：DPO+PE 优于 DPO-10%
- **核心贡献**：DPO+PE 优于 Pseudo-target
- **度量**：Acc 和 Brier 分别报告，差异 ≤1% 视为无实质差异

### 10.6 撤退标准
- **时间盒**：1-2 周诊断分析
- **触发条件**：DPO+PE 与 DPO-10% 无显著差异或更差（3 种子一致）
- **撤退动作**：不进入第二阶段，承认静态结构监督在偏好学习中无效
...
- **应急方案**：如果训练出现数值不稳定或震荡，对分母 $\sum_i p_i$ 使用 detach

### 11.2 数值稳定性
- **初始方案**：随机采样 + 足够大的 batch size（128-256）
- **借鉴 DAPO/GRPO**：如果出现退化（$p_i$ 全部接近 0.5 或 1），考虑分层采样
